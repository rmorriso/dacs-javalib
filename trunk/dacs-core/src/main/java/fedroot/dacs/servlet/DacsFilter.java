...
public class DacsFilter implements Filter {
    ...
    private static String DACS_AUTH_JURIS;
    private static String SESSION_USER_NAME;
	...
	ResponseWrapper wrappedResponse = new ResponseWrapper((HttpServletResponse)response);
        ...
        if(((HttpServletRequest)request).getParameter("username") == null ||
                ((HttpServletRequest)request).getParameter("username").trim().length() == 0) {
            // Check the user through DACS
            String dacsUsername = null;

            try {
//                dacsUsername = DacsUtil.resolveUsername(DACS_BASE_URI, DACS_AUTH_JURIS, (HttpServletRequest)request);
//
...
//                }
//
//                wrappedRequest.setParameter("username", new String[]{dacsUsername});
//
//            } catch (DacsException de) {
//                sendProcessingError("Failed authenticating with DACS: " + de.getMessage(), response);
            } catch (Exception e) {
//                 sendProcessingError("Unknown error occured: " + e.getMessage(), response);
            }
	...
	if (filterConfig != null) {
	    DACS_BASE_URI = filterConfig.getInitParameter("dacs_base_uri");
            DACS_AUTH_JURIS = filterConfig.getInitParameter("dacs_auth_juris");
            SESSION_USER_NAME = filterConfig.getInitParameter("session_user_name");
	}
    }
	...
	if (filterConfig == null) return ("DacsFilter()");
	...
	public void setParameter(String name, String []values) {
	    if (debug) System.out.println("DacsFilter::setParameter(" + name + "=" + values + ")" + " localParams = "+ localParams);
	...
	public String getParameter(String name) {
	    if (debug) System.out.println("DacsFilter::getParameter(" + name + ") localParams = " + localParams);
	    if (localParams == null)
	...
	public String[] getParameterValues(String name) {
	    if (debug) System.out.println("DacsFilter::getParameterValues(" + name + ") localParams = " + localParams);
	    if (localParams == null)
	...
	public Enumeration getParameterNames() {
	    if (debug) System.out.println("DacsFilter::getParameterNames() localParams = " + localParams);
	    if (localParams == null)
	...
	public Map getParameterMap() {
	    if (debug) System.out.println("DacsFilter::getParameterMap() localParams = " + localParams);
	    if (localParams == null)
